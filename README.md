...
可玩的 Web 图文修仙原型。玩家创建自己的角色；林晚、周安是世界中的独立 NPC。当前交付为 **V0.1 本地候选版及维护性、规模、移动端改进**。本轮变化和实测结果见 [改进验收报告](docs/reports/MAINTAINABILITY-SCALE-MOBILE.md)，原玩法验收保留在 [V0.1 报告](docs/reports/V01-LOCAL-ACCEPTANCE.md)。真实 iPhone、Windows、独立评审／试玩和真实 LLM 供应方尚未验收，未进行远端部署。
...
- 一屏创角：桌面并列身份、全身立绘与机缘，手机切换同页面板，开始按钮常驻。姓名、性别、容貌、身材、身高、胸腰臀围、模式、三件法宝与无限资质重掷；创角草稿自动保存及文件导入导出。
- 玩家与 NPC 支持主动随机生成全身立绘，读取已保存的生图模型；NPC 资料展示体貌与真实年龄。女性采用年轻成年驻颜外貌，按角色稳定组合五官、发型、衣装与妩媚姿态。默认形貌命中已生成图库时直接显示全身图；其他形貌需配置生图服务。已通过用户提供服务的 gpt-image-2 实际出图及页面采用。
...
- 游戏内「AI 模型设置」：创角页右上角和游戏「存档与设置」均可进入，分别配置 LLM 与生图模型的地址、模型 ID、密钥，支持保存、关闭和测试预览。密钥保存在 Node 服务端私有目录，当前浏览器独立使用。
...
- 原九张展示图片约 1.36 MB；默认 Seed 12345 的 40 张 NPC 全身 WebP（21 女、19 男）合计 1,716,912 字节，女性采用高开叉裙、丝袜与高跟鞋。全身图和图集均按需缓存；原始画稿和官方游戏内容锁保持不变。
...
存档仅在当前浏览器和来源（域名／端口）内保存。当前 schema 6 新增身形和立绘引用，沿用 schema 5 的紧凑结构：将知情记录按事件索引，完整命令回执保留最近 128 条，另保留累计校验链和全部命令 ID；旧档副本迁移并保留原始备份。五份十年旧档迁移后导出约 4.62–4.65 MB。仍使用世界快照，未实现完整命令日志存档或云同步，历史事件和命令 ID 仍会增长；人生存档导入上限为 16 MiB。换浏览器或端口前导出 JSON，再在新页面确认导入。新局、导入、恢复均保留被替换的旧档；不要用清空浏览器数据修复读档问题。

金丹以上、完整恋爱／道侣、宗门经营、云存档、多人、自动批量人物生图、任意脚本与完整 Mod 编辑器仍在本版之外。当前一个包 ID 只登记一个版本；新内容用于新局，不支持进行中存档热装卸或自动升级支线。未知锁会保留原档并提示恢复对应旧构建。未命中全身图库且没有自定义立绘时显示明确占位，列表保留旧缩略图（年轻女性图池会复用）；生成图片独立缓存于浏览器（48 MiB 上限），JSON 存档仅含图片哈希，换浏览器或缓存清理后需要重新生成；支线两张未完成插图明确使用占位。
