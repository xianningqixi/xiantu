...
export function Creation({
  ...
  const [profile, setProfile] = useState<Profile>(
    () =>
      initialDraft?.profile ?? {
        ...
        physique: defaultPhysique("female"),
      },
  );
  ...
  useEffect(() => {
    const value = { seed: Number(seed), roll, profile, contentLocks };
  ...
  const update = <K extends keyof Profile>(key: K, value: Profile[K]) =>
    setProfile((p) => {
      ...
      if (["sex", "appearance", "physique"].includes(key)) delete next.portraitId;
      if (key === "sex") next.physique = defaultPhysique(value as Profile["sex"]);
      return next;
    });
  const body = profilePhysique(profile);
  const setBody = <K extends keyof Physique>(key: K, value: Physique[K]) =>
    update("physique", { ...body, [key]: value });
  const reroll = () => {
    const n = roll + 1;
    setRoll(n);
    ...
    >
        ...
        <section className="creation-identity" aria-label="身份与身形">
          ...
          <div className="appearance-row">
            {(
              [
                ...
                { key: "color", label: "衣着", options: COLORS },
              ] as const
            ).map((f) => (
              <label className="form-field" key={f.key}>
                <span>{f.label}</span>
                <Select
                  value={String(profile.appearance[f.key])}
                  onValueChange={(v) =>
                    update("appearance", { ...profile.appearance, [f.key]: Number(v) })
                  }
                >
          ...
          <div className="physique-fields">
            <label className="form-field">
              ...
              <select
                value={body.build}
                onChange={(e) => setBody("build", e.target.value as Physique["build"])}
              >
            ...
            {(
              ...
              <label className="form-field" key={key}>
                ...
                <input
                  ...
                  max={max}
                  value={body[key]}
                  onChange={(e) => setBody(key, Number(e.target.value))}
                />
              </label>
            ))}
          </div>
          <p className="physique-note">
            成年修士形貌 · 外貌约 {body.apparentAge} 岁。身形只影响资料与立绘。
          </p>
        </section>
          ...
          <fieldset className="artifact-field">
            ...
            >
              {ARTIFACTS.map((a) => (
                ...
                >
                  <RadioGroupItem value={a.id} className="artifact-radio" />
                  <span className="artifact-seal serif">{a.glyph}</span>
                  <span>
      ...
      <div className="creation-bottom">
        ...
        <Button
          ...
          type="submit"
          disabled={
            busy || submitting || !profile.name.trim() || !physiqueSchema.safeParse(body).success
          }
        >
      ...
      <details className="creation-file-options">
        <summary>创角草稿文件</summary>
      ...
      <p className="save-footnote" role="status">
        {draftStatus || "进度保存在当前浏览器，可在游戏中导出备份。"}
      </p>
    </form>
  );
}
