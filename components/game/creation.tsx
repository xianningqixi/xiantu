...
export function Creation({onCreate,busy,onCancel,initialDraft,onSaveDraft}:{onCreate:(profile:Profile,seed:number)=>void|Promise<void>;busy:boolean;onCancel?:()=>void;initialDraft?:CreationDraft|null;onSaveDraft?:(draft:Omit<CreationDraft,'revision'|'version'>)=>Promise<CreationDraft>}){
  ...
  const saved=useRef(JSON.stringify({seed:Number(seed),roll,profile}));
  useEffect(()=>{
    const value={seed:Number(seed),roll,profile};const signature=JSON.stringify(value);
    ...
    return()=>{active=false;};
  },[seed,roll,profile,onSaveDraft]);
  const submit=async()=>{
    if(submitLock.current)return;submitLock.current=true;setSubmitting(true);
    try{if(onSaveDraft)await onSaveDraft({seed:Number(seed),roll,profile});await onCreate(profile,Number(seed));}
  ...
  return <form className="creation-form" onSubmit={e=>{e.preventDefault();void submit();}}>
    ...
    <div className="creation-bottom">{onCancel&&<Button variant="ghost" type="button" onClick={onCancel}>回到此世</Button>}<Button className="begin-button" type="submit" disabled={busy||submitting||!profile.name.trim()}>{busy||submitting?'正在展开这一世…':'踏入仙途'}<ArrowRight size={18}/></Button></div>
    <p className="save-footnote" role="status">{draftStatus||'进度保存在当前浏览器，可在游戏中导出备份。'}</p>
  </form>;
}
