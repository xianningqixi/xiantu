...
本卷 27 张 PNG 已通过 art/manifest.json 接入，所有视觉槽位 owned，女角色 portraitId 已登记。派生 WebP 共 2,573,076 字节，全部 lazy；图片摘要与锁保存在本卷 integrity.json，原 storylets／outline／art-prompts／PNG 未改。
...
本卷当前 lock：`shichai.qiudeng@0.1.0:3df1e31175bba760f48101c664826ad0a8def2ec75761d6f7468d25db04c71e4`。
...
| 检查 | 结果 | 说明 |
| --- | --- | --- |
| `npm run content:prepare` 严格协议校验与注册 | PASS，退出码 0 | 节点 31、标记 28、角色 5、视觉槽位 27（全部 owned，已接入） |
...
| 路线 | 预期流程 | 状态 |
| --- | --- | --- |
| 闻人霜（情敌 崔九章） | 相识→相知→心动→情敌介入→初夜→双修→道侣；退让分支进入被夺结局；身故分支收束 | 道侣路线规则测试通过；完整浏览器分支未逐条试玩 |
| 江照雪（情敌 白无咎） | 相识→相知→心动→情敌介入→初夜→双修→道侣；退让分支进入被夺结局；身故分支收束 | 道侣路线规则测试通过；完整浏览器分支未逐条试玩 |
