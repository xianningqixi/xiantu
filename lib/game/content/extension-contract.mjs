...
export const extensionSchema = z
  .object({
          ...
          .array(
            z
              .object({
                ...
                sect: text,
                goal: text,
                location: z.enum(["market", "inn", "gate"]),
              })
              .strict(),
          )
          .max(10),
    ...
    visuals: z.record(
      id,
      z.object({ assetId: id, alt: text, status: z.enum(["reused", "planned"]) }).strict(),
    ),
    storylets: z
      .array(
        z
...
export function validateExtension(input, official) {
  ...
  for (const [key, v] of Object.entries(p.visuals))
    check(
      v.status === "planned" || official.assets.includes(v.assetId),
  ...
  for (const n of p.storylets) {
    ...
    for (const c of n.conditions) {
      ...
      if (c.fact === "location")
        check(["market", "inn", "gate", "ruins"].includes(c.value), `${n.id}.location 未知`);
    }
