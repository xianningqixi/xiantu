...
export const LOCATIONS = data.locations as Record<LocationId, { name: string; subtitle: string; body: string; destinations: LocationId[]; visualId: string }>;
export const STORY = data.story as StoryNode[];
export function visual(id: string) { return data.art.assets[id] || { url: '', alt: '此处图景，待你想象。', kind: 'scene' }; }
export const ART = {
  market: visual(LOCATIONS.market.visualId).url, inn: visual(LOCATIONS.inn.visualId).url,
  gate: visual(LOCATIONS.gate.visualId).url, ruins: visual(LOCATIONS.ruins.visualId).url,
...
export function contentText(value: string, w: World) {
  const tokens: Record<string, string> = {
    ...
    'primary.name': w.npcs.find(a => a.id === PACK.roles.primary)?.name || CHARACTERS.primary.name,
    'companion.name': w.npcs.find(a => a.id === PACK.roles.companion)?.name || CHARACTERS.companion.name,
  };
  return value.replace(/\{\{([^{}]+)\}\}/g, (_, key: string) => tokens[key] ?? '未知人物');
}

export const ARTIFACTS = [{"id": "focus", "name": "定心佩", "glyph": "定", "description": "心静则道生。每日修炼额外获得 2 点修为。"}, {"id": "ward", "name": "归途符", "glyph": "归", "description": "山长水远，总有归途。撤退成功率提高 15%。"}, {"id": "bond", "name": "同心绳", "glyph": "缘", "description": "一诺千金。履行约定时，同伴额外增加信任。"}] as const;
