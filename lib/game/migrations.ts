...
export function migrateSave(value: unknown): { world: World; migrated: boolean } {
  ...
  if(version!==4){copy.schemaVersion=4;copy.negotiations=[];if(version!==3){copy.contentLocks=[];copy.contentState={};}else{copy.contentLocks??=[];copy.contentState??={};}}
  if (version !== 3 && version !== 4) {
    ...
    if(copy.longAction){const a=copy.longAction;a.id=`legacy-action:${copy.day-a.total+a.remaining}`;a.checkpoint=a.total-a.remaining;a.paidStones=a.kind==='train'&&a.stoneMethod?a.checkpoint:0;}
  }
