...
function initialAge(w:World,a:Actor){
  const elapsed=a.alive?w.day:(w.events.find(e=>e.kind==='death'&&e.actors.includes(a.id))?.day??w.day);
...
export function npcPortrait(w:World,a:Actor):{src:string;slot:number|null}{
  ...
  const peers=w.npcs.filter(n=>n.sex===a.sex&&n.id!==PACK.roles.primary&&n.id!==PACK.roles.companion).sort((x,y)=>(x.appearanceSeed>>>0)-(y.appearanceSeed>>>0)||x.id.localeCompare(y.id));
  const reserved=Object.values(presentation.fixed).map(p=>p.portrait);
  const pool=presentation.atlases.filter(atlas=>atlas.sex===a.sex).flatMap(atlas=>atlas.slots.map(slot=>({src:atlas.url,slot:slot.index,age:slot.age}))).filter(p=>!reserved.some(r=>r.src===p.src&&r.slot===p.slot));
  let available=pool.slice();
  for(const peer of peers){
    if(!available.length)available=pool.slice();
    const age=initialAge(w,peer);
    let nearest=0;
    for(let i=1;i<available.length;i++)if(Math.abs(available[i].age-age)<Math.abs(available[nearest].age-age))nearest=i;
