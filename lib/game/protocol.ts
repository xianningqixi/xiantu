...
export const profileSchema = z
  .object({
    ...
    artifact: z.enum(["focus", "ward", "bond"]),
...
const commandSchemas = [
  ...
  z
    .object({
      ...
      action: z.enum(["attack", "skill", "guard", "heal", "retreat"]),
      target: id.optional(),
    })
    .strict(),
...
const requestSchema = z.union([
  ...
  z
    .object({
      ...
      text: z.string().max(balanceLimits.limits.maxImportBytes),
      replace: z.boolean().optional(),
      expected: expectedSchema,
    })
    .strict(),
...
export function parseRequest(value: unknown): WorkerRequest {
  ...
  if (!result.success)
    throw new GameError("VALIDATION_ERROR", "行动数据不完整或含不支持的字段，请重新读取后重试。");
  return result.data as WorkerRequest;
}
export function parseCommand(value: unknown): Command {
  const result = commandSchema.safeParse(value);
  if (!result.success)
    throw new GameError("VALIDATION_ERROR", "行动参数不合法，不会消耗时间或资源。");
