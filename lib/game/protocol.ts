...
const commandSchemas = [
  ...
  ...(['step', 'stop', 'work', 'rest', 'learn', 'expedition', 'return', 'disband', 'compensate', 'exchange', 'heal', 'formParty', 'rally'] as const).map(type => z.object({ type: z.literal(type) }).strict()),
...
export const draftSchema = z.object({ version: z.literal(1), revision: integer, seed: integer.max(4294967295), roll: integer.max(1000000), profile: profileSchema }).strict();
...
const requestSchema = z.union([
  ...
  z.object({ ...envelope, kind: z.literal('create'), profile: profileSchema, seed: integer.max(4294967295), replace: z.boolean().optional(), expected: expectedSchema }).strict(),
  z.object({ ...envelope, kind: z.literal('command'), command: commandSchema, revision: integer, expected: expectedSchema }).strict(),
  z.object({ ...envelope, kind: z.literal('import'), text: z.string().max(5 * 1024 * 1024), replace: z.boolean().optional(), expected: expectedSchema }).strict(),
