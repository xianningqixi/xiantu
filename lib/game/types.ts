...
export interface Actor {
  ...
  alive: boolean; location: LocationId; activity: string; readyDay: number; lastActionDay: number;
  attempt: { remaining: number; chance: number } | null;
}
export interface Relation { from: string; to: string; favor: number; trust: number; attraction: number; known: boolean; memories: string[]; socialEventId?: string; grievance?: boolean; }
export interface WorldEvent { id: string; day: number; kind: string; text: string; actors: string[]; public: boolean; location?: LocationId; count?: number; lastDay?: number; }
export interface Knowledge { eventId: string; knower: string; source: 'participant' | 'witness' | 'told' | 'public' | 'legacy'; sourceActor: string | null; learnedDay: number; }
...
export interface Agreement { id: string; status: 'accepted' | 'active' | 'fulfilled' | 'breached' | 'not_triggered' | 'cancelled' | 'impossible'; members: string[]; recipient: string; expeditionId: string | null; strict: boolean; meeting?: { location: LocationId }; reason?: string; }
...
export interface Battle { id: string; round: number; allies: Fighter[]; enemies: Fighter[]; logs: string[]; auto: boolean; lethal: boolean; }
export interface LongAction { id: string; checkpoint: number; paidStones: number; kind: 'train' | 'wait' | 'breakthrough'; total: number; remaining: number; stoneMethod: boolean; chance: number; guardian: string | null; }
export interface World {
  ...
  format: 'xiantu-web-1'; rulesVersion: '0.1.2'; packLock: string; saveId: string; revision: number;
...
export type Command =
  | {type:'adoptNegotiation';proposalId:string;sessionId:string;saveId:string;revision:number;target:string;proposal:NegotiationProposal;confirmed:true}
  ...
  | { type: 'step' }
  | { type: 'stop' }
    | { type: 'work' | 'rest' | 'learn' | 'expedition' | 'return' | 'disband' | 'compensate' | 'exchange' | 'heal' | 'resolveAgreement' }
...
export interface CreationDraft { version: 1; revision: number; seed: number; roll: number; profile: Profile; contentLocks?: string[]; }
export interface BackupSummary { key: string; name: string; day: number; revision: number; }
export interface WorkerRequest { id: string; protocolVersion?: 1; kind: 'load' | 'create' | 'command' | 'import' | 'export' | 'loadDraft' | 'saveDraft' | 'backups' | 'restore' | 'exportBackup'; revision?: number; expected?: SaveExpectation; profile?: Profile; seed?: number; contentLocks?: string[]; command?: Command; text?: string; replace?: boolean; draft?: CreationDraft; backupKey?: string; }
