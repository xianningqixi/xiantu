...
async function imageTest(request: Request, config: StoredModel, fetcher: typeof fetch) {
  ...
  try {
    ...
    const response = await fetcher(`${config.baseUrl}/images/generations`, {
      ...
      body: JSON.stringify({
        model: config.model,
        prompt: "中国古风山水，一座青石小镇，远山与薄雾，无文字。",
        n: 1,
        size: "1024x1024",
      }),
    });
    ...
    try {
      for (;;) {
        ...
        if (done) break;
        size += value.length;
        if (size > 6 * 1024 * 1024) throw new Error("图片过大。");
        chunks.push(value);
      }
    } finally {
  ...
  try {
    ...
    if (input.kind === "image") {
      try {
        return reply(200, {
          message: "生图成功，仅作预览。",
          image: await imageTest(request, config, fetcher),
        });
      } catch {
