...
type Asset = { id: string; blob: Blob; signature: string; savedAt: number };
...
export async function cachePortrait(dataUrl: string, signature: string) {
  ...
  try {
    await new Promise<void>((resolve, reject) => {
      ...
      all.onsuccess = () => {
        ...
        let total = previous
          .filter((a) => a.id !== id)
          .reduce((sum, a) => sum + a.blob.size, blob.size);
        ...
        store.put({ id, blob, signature, savedAt: Date.now() });
      };
