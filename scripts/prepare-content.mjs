...
const p = validateContent({ manifest: read('manifest.json'), story: read('storylets.json'), locations: read('locations.json'), characters: read('characters.json'), presentation: read('presentation.json'), art: read('art/manifest.json') });
...
fs.writeFileSync(path.join(root, 'integrity.json'), JSON.stringify({ contentHash: digest, lock: `${p.manifest.id}@${p.manifest.version}:${digest}`, images: hashes }, null, 2) + '\n');
...
if (!Array.isArray(npc.backgrounds) || !npc.backgrounds.length || !Array.isArray(npc.atlases) || !npc.atlases.length) throw new Error('NPC 展示包结构不完整');
for (const bio of [...npc.backgrounds, ...Object.values(npc.fixed)]) for (const field of ['origin','background','interest','wish']) if (typeof bio[field] !== 'string' || !bio[field].trim()) throw new Error(`NPC 资料缺少 ${field}`);
const npcHashes = {};
const urls = new Set(Object.values(p.art.assets).map(a=>a.url));
for (const atlas of npc.atlases) {
  if (!['male','female'].includes(atlas.sex) || atlas.columns !== 3 || atlas.rows !== 3 || atlas.slots.length !== 9 || new Set(atlas.slots.map(s=>s.index)).size !== 9 || atlas.slots.some(s=>!Number.isInteger(s.index)||s.index<0||s.index>8||!Number.isInteger(s.age)||s.age<18)) throw new Error(`NPC 图集格位不合法：${atlas.id}`);
  if (!/^art\/images\/[a-z0-9-]+\.png$/.test(atlas.file) || !/^\/art\/[a-z0-9-]+\.png$/.test(atlas.url) || urls.has(atlas.url)) throw new Error(`NPC 图片路径重复或不合法：${atlas.id}`);
  const source = path.join(root, atlas.file); const bytes = fs.readFileSync(source);
  if (bytes.subarray(0,8).toString('hex') !== '89504e470d0a1a0a' || bytes.readUInt32BE(16) !== atlas.width || bytes.readUInt32BE(20) !== atlas.height || atlas.width % 3 || atlas.height % 3) throw new Error(`NPC 图集尺寸不匹配：${atlas.id}`);
  npcHashes[atlas.id] = sha(bytes); urls.add(atlas.url);
  fs.copyFileSync(source, path.join(project, 'public', atlas.url));
}
for (const bio of Object.values(npc.fixed)) if (!urls.has(bio.portrait.src) || (bio.portrait.slot !== null && (!Number.isInteger(bio.portrait.slot)||bio.portrait.slot<0||bio.portrait.slot>8))) throw new Error('固定 NPC 立绘引用无效');
fs.writeFileSync(path.join(root,'npc-presentation-integrity.json'), JSON.stringify({version:npc.version, hash:sha(JSON.stringify({npc,images:npcHashes})),images:npcHashes},null,2)+'\n');

// A small, deterministic ZIP writer (STORE method); no system zip utility or extra dependency.
function crc32(data) { let crc = 0xffffffff; for (const byte of data) { crc ^= byte; for (let k = 0; k < 8; k++) crc = (crc >>> 1) ^ ((crc & 1) ? 0xedb88320 : 0); } return (crc ^ 0xffffffff) >>> 0; }
function files(dir, prefix = '') { return fs.readdirSync(dir, { withFileTypes: true }).sort((a,b) => a.name.localeCompare(b.name, 'en')).flatMap(entry => entry.isDirectory() ? files(path.join(dir, entry.name), prefix + entry.name + '/') : [prefix + entry.name]); }
const local = [], central = []; let offset = 0;
const names = files(root);
for (const name of names) {
  const filename = Buffer.from('official-qingshi/' + name);
  const bytes = fs.readFileSync(path.join(root, name)); const crc = crc32(bytes);
  ...
  const index = Buffer.alloc(46); index.writeUInt32LE(0x02014b50); index.writeUInt16LE(20, 4); index.writeUInt16LE(20, 6); index.writeUInt16LE(0x800, 8); index.writeUInt16LE(33, 14); index.writeUInt32LE(crc, 16); index.writeUInt32LE(bytes.length, 20); index.writeUInt32LE(bytes.length, 24); index.writeUInt16LE(filename.length, 28); index.writeUInt32LE(offset, 42);
  central.push(index, filename); offset += header.length + filename.length + bytes.length;
}
const end = Buffer.alloc(22); end.writeUInt32LE(0x06054b50); end.writeUInt16LE(names.length, 8); end.writeUInt16LE(names.length, 10); end.writeUInt32LE(central.reduce((n,b)=>n+b.length,0), 12); end.writeUInt32LE(offset, 16);
