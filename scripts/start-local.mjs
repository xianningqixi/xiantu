...
const {values} = parseArgs({options:{port:{type:'string',short:'p'},hostname:{type:'string',short:'H'}}});
const port = Number(values.port ?? process.env.PORT ?? 3000);
if (!Number.isInteger(port) || port < 1 || port > 65535) throw new Error('Invalid port');
const {server} = await startProdServer({port,host:values.hostname ?? '127.0.0.1',outDir:'dist'});
const handlers = server.listeners('request');
server.removeAllListeners('request');
server.on('request',(request,response)=>{
  try { request.url = originTarget(request.url ?? '/', request.headers.host); }
