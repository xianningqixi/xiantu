  ...
  await page.addInitScript(() => {
    ...
    window.Worker = class extends NativeWorker {
      constructor(url: URL | string, options?: WorkerOptions) {
        super(url, options);
        (window as any).__xiantuWorker = this;
        this.addEventListener("message", (event) => {
          if (event.data?.__xiantuFaultHit) (window as any).__xiantuFaultHit = true;
        });
      }
    };
  });
  await page.route(
    (url) => /simulation\.worker.*\.(?:js|ts)$/.test(url.pathname),
    async (route) => {
      const response = await route.fetch();
      await route.fulfill({
        response,
        body: injection + (await response.text()),
...
async function fault(page: Page, type: string) {
  await page.evaluate(
    (type) => (window as any).__xiantuWorker.postMessage({ __xiantuTestFault: type }),
    type,
  );
}
for (const type of ["abort", "quota"])
  test(`real IndexedDB ${type} keeps the old save and retry commits once`, async ({ page }) => {
    await setup(page);
...
for (const type of ["beforePut", "lostAck"])
  test(`worker ${type} recovers the last complete checkpoint after reopening`, async ({ page }) => {
    await setup(page);
    const before = await saved(page);
    await fault(page, type);
    await page.getByRole("button", { name: /接些坊市杂务/ }).click();
...
test("another IndexedDB schema upgrade closes the worker connection and permits a reload", async ({
  page,
}) => {
  await setup(page);
  const before = await saved(page);
  await page.evaluate(
    () =>
      new Promise<void>((resolve, reject) => {
        ...
        q.onblocked = () => reject(new Error("Worker failed to close its old connection"));
      }),
  );
  // A future physical database version is intentionally not downgraded by the old runtime.
  await page.reload();
  await expect(page.getByRole("alert")).toContainText("本机存档");
  expect(await saved(page)).toEqual(before);
});

for (const type of ["beforePut", "lostAck"])
  test(`paid long action ${type} at checkpoint four resumes without duplicate days or cost`, async ({
    page,
  }) => {
    await setup(page);
    await page.getByRole("button", { name: /接些坊市杂务/ }).click();
