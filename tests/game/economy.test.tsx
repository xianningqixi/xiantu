...
test("rendered shop prices equal the actual rule debit for every item", () => {
  for (const item of Object.keys(ALL_SHOP_ITEMS) as (keyof typeof ALL_SHOP_ITEMS)[]) {
...
test("rendered sale prices equal actual credits for every sellable item", () => {
  for (const item of ["healing", "pills", "grass"] as const) {
    const world = createWorld(42, profile, `sale-${item}`);
    world.player[item] = 2;
    const html = renderToStaticMarkup(
      <InventoryPanel world={world} send={async () => true} busy={false} />,
    );
    const displayed = Number(
      html.match(new RegExp(`data-sell-item="${item}" data-price="(\\d+)"`))?.[1],
    );
    assert.ok(displayed > 0);
    ...
    assert.equal(next.player.stones - world.player.stones, displayed);
    assert.equal(next.player[item], 1);
  }
});
