...
export function simulationFingerprint(world:World):string {
  ...
  const aliases=new Map(events.map(({id,...event})=>[id,createHash('sha256').update(canonicalJson(event)).digest('hex')]));
  ...
  return createHash('sha256').update(canonicalJson(replace({...state,events,knowledge:world.knowledge.filter(k=>aliases.has(k.eventId))}))).digest('hex');
}
