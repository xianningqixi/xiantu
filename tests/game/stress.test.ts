...
for (const seed of B.world.stressTestSeeds)
  test(`seed ${seed}: 100 NPC / 3650 days with checkpoint recovery`, () => {
    let w = createWorld(seed, profile, `stress:${seed}`, 100),
      serial = 0;
    ...
    const command = (c: Command) => {
      ...
      try {
        w = applyCommand(w, c, `stress:${++serial}`, w.revision);
      } catch (error) {
        writeFileSync(
          `${directory}/failure-${seed}.json`,
          JSON.stringify({
            seed,
            day: w.day,
            revision: w.revision,
            command: c,
            recent: w.events.slice(-10),
            error: String(error),
          }),
        );
        throw error;
      ...
      if (w.day % 365 === 0)
        console.log(
          JSON.stringify({
            seed,
            day: w.day,
            seconds: +((performance.now() - began) / 1000).toFixed(2),
          }),
        );
    }
    validateWorld(w);
    assert.equal(w.npcs.length, 100);
    ...
    const report = {
      ...
      prettyExportBytes: Buffer.byteLength(JSON.stringify(w, null, 2)),
      rules: w.rulesVersion,
      schema: w.schemaVersion,
      packLock: w.packLock,
      days: w.day,
      commands: serial,
      alive: w.npcs.filter((n) => n.alive).length,
      dead: w.npcs.filter((n) => !n.alive).length,
      bytes: Buffer.byteLength(json),
      seconds: +((performance.now() - began) / 1000).toFixed(2),
      maxCommandMs: +maxMs.toFixed(2),
      fingerprint: digest(w),
      checkpointRecovery: "passed",
    };
    writeFileSync(`${directory}/${seed}.json`, JSON.stringify(report, null, 2));
    writeFileSync(`${directory}/save-${seed}.json`, json);
